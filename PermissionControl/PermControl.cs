...
public class PermControl : TerrariaPlugin
{
    ...
    public void SCommand(CommandArgs args)
    {
        ...
        {
            List<string> commandNameList = new();
    ...
    public void SearchCommandInGroup(CommandArgs args)
    {
        ...
        List<Command> therealcommand = new();
    ...
    private void SearchPermInGroup(CommandArgs args)
    {
        ...
        var groupsWithDirectPermissions = TShock.Groups.Where(e => e.Permissions.Contains(perms));
        var groupsWithInheritedPermissions = TShock.Groups.Where(e => e.HasPermission(perms));

        args.Player.SendInfoMessage($"Direct groups with the {perms} permission: {string.Join(", ", groupsWithDirectPermissions.Select(e => e.Name))}");
        args.Player.SendInfoMessage($"All groups with the {perms} permission: {string.Join(", ", groupsWithInheritedPermissions.Select(e => e.Name))}");
    }
    #endregion
    ...
    {
        List<string> plugincommands = new();
